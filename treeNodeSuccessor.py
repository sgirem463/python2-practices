...
def findMIn(current):
    if (current.left):
        ...
        print 'found:', current, 'key:', current.kay
        return current

def findLeftAncestor(current):
    if not current.parent:
        ...
        if current.parent.left == current:
            ...
            return findLeftAncestor(current.parent)

def findSuccessor(current):
    if current.right:
        return findMin(current.right)
    elif current.parent:
        if current.parent.left == current:
            return current.parent
        else: # curent.parent.right == current #is a right child, go up until a anscestor is a left child, or no parent
            leftAnscetor = findLeftAncestor(current.parent)
            if leftAnscetor:
                return findMIn(leftAnscetor)
            else:
                return None
